...
license: MIT
compatibility: Requires Node.js 20+. Optional sidecars use ANTHROPIC_API_KEY, OPENAI_API_KEY, NOVITA_API_KEY, or an installed claude, codex, gemini, or pi CLI.
...
npx --yes semantic-review --with novita --model deepseek/deepseek-v3.2
...
For example, “use Claude Code with fable” means `--with claude --model fable`. The available backends are `anthropic`, `openai`, `novita`, `claude`, `codex`, `gemini`, and `pi`. CLI backends reuse their existing local authentication; the API backends use `ANTHROPIC_API_KEY`/`ANTHROPIC_AUTH_TOKEN`, `OPENAI_API_KEY`, and `NOVITA_API_KEY`. If the user names a backend without a model, leave off `--model` and let that backend choose its default.
