...
license: MIT
compatibility: Requires Node.js 20+. Optional sidecars use ANTHROPIC_API_KEY, NOVITA_API_KEY, or an installed claude, codex, gemini, or pi CLI.
...
For example, “use Claude Code with fable” means `--with claude --model fable`. The available harnesses are `anthropic`, `novita`, `claude`, `codex`, `gemini`, and `pi`. CLI harnesses reuse their existing local authentication; `anthropic` uses `ANTHROPIC_API_KEY` or `ANTHROPIC_AUTH_TOKEN`; `novita` uses `NOVITA_API_KEY`. If the user names a harness without a model, leave off `--model` and let that backend choose its default.
